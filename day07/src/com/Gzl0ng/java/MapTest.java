 ...
 * 3.CurrentHashMap 与 HashTable的异同？（暂时不讲  分段锁技术）
 *
 * 二，Map结构的理解
 *  Map中的key：无序的，不可重复的，使用Set存储所有的key ---> key所在的类要重写equals（）和 hashCode（）（以、hashMap为例）
 *  Map中的value：无序的，可重复的，使用Collection存储所有的value --->value所在的类要重写equals（）
 *  一个键值对：key-value构成能了一个Entry对象。
 *  Map中的entry：无序的，不可重复的，使用Set存储所有的entry
 *
 *  三，HashMap的底层实现原理？以jdk7为例说明：
 *  HashMap map = new HashMap（）：
 *  在实例化以后，底层创建了长度为16的数组Entry[] table。
 *  ...可能多次执行过多次put...
 *  map.put（key1，value1）：
 *  首先，调用key1所在类的hashCode（）计算key1哈希值，此哈希值经过某种算法计算以后，得到在Entry数组中的存放位置（与15）
 *  如果此位置上的数据为空，此时的key1-value1添加成功。 ----情况1
 *  如果此位置上的数据不为空，（意味着此位置存在一个或多个数据（以链表形式存在）），比较key1和已经存在的一个或多个数据
 *  的哈希值：
