...
public class StringTest {
    ...
    public void test3(){
        ...
        String s8 = s6.intern();//返回得到的s8是使用常量池中已经存在的javaEE和hadoop
    ...
    面试题：String s = new String（“abc”）；方式创建对象，在内存中创建了几个对象？
            二个：一个是推空间中newjieg，另一个是char[]对应的常量池中的数据：“abc”.
     */
    @Test
    public void test2(){
        //通过字面量定义非方式：此时的s1和s2的数据javaEE声明在方法区中的字符串常量池中
        String s1 = "javaEE";
        String s2 = "javaEE";
    ...
    4.String：代表一个不可变的字符序列。简称：不可变性。
        体现：1.当字符串重新赋值时，需要重写指定内存区域赋值，不能使用原有的value进行赋值。
             ...
             3.当调用String的replace（）方法修改字符串时，也需要指定内存区域赋值。
